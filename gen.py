...
for x in range(len(type_hits)):
    print("    \"" + translate(type_hits[x][0]) + "\": " +  type_hits[x][1] + ("," if x < len(type_hits) - 1 else ""))
...
lt_hits = re.findall(r' *lt([A-Z_]+)[^\n=]*= *([^,]+),?$', ledgerformats_h, re.MULTILINE)
for x in range(len(lt_hits)):
    if lt_hits[x][0] == 'ANY':
        print("    \"" + translate(lt_hits[x][0])  + "\": " + "-3" + ("," if x < len(lt_hits) - 1 else ""))
    elif lt_hits[x][0] == 'CHILD':
        print("    \"" + translate(lt_hits[x][0])  + "\": " + "-2" + ("," if x < len(lt_hits) - 1 else ""))
    else:
        print("    \"" + translate(lt_hits[x][0])  + "\": " + unhex(lt_hits[x][1]) + ("," if x < len(lt_hits) - 1 else ""))
print('  },')
print('  "FIELDS": [')
print("""    [
      "Generic",
...
hits = re.findall(r'^ *CONSTRUCT_[^\_]+_SFIELD *\( *[^,\n]*,[ \n]*"([^\"\n ]+)"[ \n]*,[ \n]*([^, \n]+)[ \n]*,[ \n]*([0-9]+)(,.*?(notSigning))?', sfield_cpp, re.MULTILINE)
...
hits = re.findall(r'^ *((tel|tem|tef|ter|tes|tec)[A-Z_]+)( *= *([0-9-]+))? *,? *(\/\/[^\n]*)?$', ter_h, re.MULTILINE)
upto = -1
last = ""
for x in range(len(hits)):
    # print(hits[x])
...
hits = re.findall(r'^ *tt([A-Z_]+) *(\[\[[^\]]+\]\])? *= *([0-9]+) *,?.*$', txformats_h, re.MULTILINE)
for x in range(len(hits)):
    print('    "' + ttranslate(hits[x][0]) + '": ' + hits[x][2] + ("," if x < len(hits) - 1 else ""))
